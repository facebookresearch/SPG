...
for task in "${TASKS[@]}"; do
  for gen_length in "${GEN_LENGTHS[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=$GPU_LIST torchrun \
      ...
      --output_dir "eval_results/eval_results_${task}_llada1.5" \
      --model_path "${SAVE_DIR}/hf_models/LLaDA-1.5/" 
  done
done
...
for gen_length in "${GEN_LENGTHS[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES=$GPU_LIST torchrun \
    ...
    --gen_length $gen_length \
    --few_shot 3 \
    --output_dir "eval_results/eval_results_sudoku_new_3shot_llada1.5" \
    --model_path "${SAVE_DIR}/hf_models/LLaDA-1.5/" 
done
