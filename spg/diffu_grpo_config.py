...
class DiffuGRPOConfig(TrainingArguments):
    ...
    trainer: Optional[str] = field(
        default="spg",
        metadata={"help": "Trainer to use. 'diffu_grpo' or 'spg'."},
    )
    use_mask_prompt: bool = field(
        default=True,
        metadata={"help": "Whether to use mask prompt."},
    )
    # Parameters that control the spg trainer
    forward_type: Optional[str] = field(
        default="all",
        metadata={"help": "Type of forward process. 'all' or 'random' or 'block_all' or 'block_random'."},
    )
    num_t: Optional[int] = field(
        default=1,
        metadata={"help": "Number of timesteps to sample for each mc estimation."},
    )
    min_t: Optional[float] = field(
        default=0,
        metadata={"help": "Minimum timestep to sample."},
    )
    max_t: Optional[float] = field(
        default=1,
        metadata={"help": "Maximum timestep to sample."},
    )
    logp_estimation: Optional[str] = field(
        default=None,
        metadata={"help": "Type of logp estimation for negative advantage traces. eubo or mix or elbo or zero."},
    )
    eubo_beta: Optional[float] = field(
        default=1.5,
        metadata={"help": "Beta parameter for Eubo estimation."},
    )
    mix_weight: Optional[float] = field(
        default=0.5,
        metadata={"help": "Weight for mixing loss between mean field and normal."},
    )
